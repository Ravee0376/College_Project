...
AVG((ans.markObtained/q.markPerQuestion)*100) AS percent
FROM registration_t AS r, answer_t AS ans, question_t AS q, 
co_t AS co, plo_t AS plo, section_t AS t
WHERE r.registrationID=ans.registrationID 
AND ans.examID=q.examID AND ans.answerNum=q.questionNum AND q.coNum=co.coNum 
AND q.courseID=co.courseID AND co.ploID=plo.ploID 
AND r.sectionID=t.sectionID AND t.facultyID='$studentID' 
AND r.studentID IN (SELECT studentID FROM registration_t WHERE sectionID IN 
(SELECT sectionID FROM section_t WHERE facultyID='$studentID'))
GROUP BY plo.ploNum,r.sectionID";
...
FROM registration_t AS r, answer_t AS ans, question_t AS q, 
co_t AS co, plo_t AS plo, section_t AS t 
WHERE r.studentID IN (SELECT studentID FROM registration_t WHERE sectionID IN 
(SELECT sectionID FROM section_t WHERE facultyID='$studentID'))
...
function drawAutumnChart() {
  console.log("drawAutumnChart() called");
  var data = google.visualization.arrayToDataTable([
    ["ploNum", "Individual", "Dept Average"],
